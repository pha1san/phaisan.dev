export default function CarouselSlider({ swiper, on, extendParams }: { swiper: any; on: any; extendParams: any }) {
  extendParams({
    carouselEffect: {
      opacityStep: 0.33,
      scaleStep: 0.2,
  ...
  on("progress", () => {
    ...
    for (let i = 0; i < swiper.slides.length; i += 1) {
      ...
      const translate = `${slideProgress * modify * translateModifier * (swiper.rtlTranslate ? -1 : 1)}%`;
  ...
  on("setTransition", (s: unknown, duration: unknown) => {
    if (swiper.params.effect !== "carousel") return;
    for (let i = 0; i < swiper.slides.length; i += 1) {
      const slideEl = swiper.slides[i];
      const opacityEls = slideEl.querySelectorAll(".swiper-carousel-animate-opacity");
      slideEl.style.transitionDuration = `${duration}ms`;
      opacityEls.forEach((opacityEl: { style: { transitionDuration: string } }) => {
